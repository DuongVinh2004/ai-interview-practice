...
2. **Execution Trust Boundary:** Workspace code execution is strictly decoupled from the core API process:
   - *Stage A (Local / CI / Unit Tests):* Deterministic Mock Sandbox Adapter that executes isolated command mockups without external VM dependencies.
   - *Stage B (Development / Pilot):* Ephemeral isolated containers (Docker) with non-root user, CPU/RAM quotas, read-only root filesystems, dropped network egress, and strict timeouts (15s).
   - *Stage C (Production):* Remote firecracker/gVisor microVMs or dedicated Judge0 multi-file cluster.
