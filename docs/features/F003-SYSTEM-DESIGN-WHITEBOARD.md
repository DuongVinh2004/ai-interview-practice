...
```
*(Nếu `triggerAi = true`, AI response sẽ được trả về qua SSE/WebSocket connection của Interview room)*
...
3. **Right Panel (20%): AI Chat & Feedback**
   - Khung chat hiển thị câu hỏi từ AI Bot (VD: *"Bạn sử dụng Redis cho cache, điều gì xảy ra nếu Redis node bị sập?"*).
