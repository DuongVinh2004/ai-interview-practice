...
export class StarRubric {
  ...
  static evaluate(answerText: string): {
    ...
    if (actionScore >= 3.5)
      strengths.push('Clear personal ownership and proactive execution steps described.');
    if (situationScore >= 3.0)
      strengths.push('Good contextual backdrop highlighting key problem constraints.');
    if (resultScore >= 3.5)
      strengths.push('Compelling quantifiable metrics demonstrating tangible business impact.');

    if (missing.includes('result')) {
      improvements.push(
        'Quantify the final outcome with measurable business or technical metrics (e.g. latency reduced by X%, revenue boosted).',
      );
    }
    if (!hasAction) {
      improvements.push(
        'Focus more on your specific personal contributions rather than general team activities ("I did" vs "We did").',
      );
    }
    if (missing.includes('task')) {
      improvements.push(
        'Clarify the specific objective and your personal responsibility in the situation.',
      );
    }

    if (strengths.length === 0) strengths.push('Articulated an authentic professional experience.');
    if (improvements.length === 0)
      improvements.push('Maintain high storytelling precision and concise delivery.');
    ...
    return {
      ...
      extracted: {
        situationText: hasSituation
          ? text.slice(0, Math.min(200, Math.floor(text.length * 0.35)))
          : undefined,
        taskText: hasTask
          ? text.slice(Math.floor(text.length * 0.25), Math.floor(text.length * 0.5))
          : undefined,
        actionText: hasAction
          ? text.slice(Math.floor(text.length * 0.45), Math.floor(text.length * 0.8))
          : undefined,
