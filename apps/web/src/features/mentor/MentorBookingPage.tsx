  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8" data-testid="mentor-booking-page">
        ...
        <div className="bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-3xl p-6 shadow-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {mySessions.slice(0, 2).map(s => (
              <div
                key={s.id}
                className="p-4 bg-white/10 rounded-2xl border border-white/10 flex items-center justify-between"
              >
                <div>
      ...
      <div
        ...
        data-testid="mentor-grid"
      >
        {isLoading ? (
          [1, 2, 3].map(i => (
            <div
              key={i}
              className="h-64 bg-slate-100 rounded-3xl animate-pulse border border-slate-200"
            />
          ))
