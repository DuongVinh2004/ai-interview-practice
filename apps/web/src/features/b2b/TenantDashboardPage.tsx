...
export const TenantDashboardPage: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8" data-testid="tenant-dashboard-page">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-sm space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">Monitor student progress, roster, and assignments per batch</p>
          </div>

          <Link to="/b2b/cohorts" className="text-xs font-bold text-emerald-700 hover:underline flex items-center gap-1">
            View All Cohorts <ArrowRight className="h-3.5 w-3.5" />
          </Link>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" data-testid="cohort-list">
          {isLoadingCohorts ? (
            [1, 2, 3].map((i) => (
              ...
              >
                ...
                <div className="mt-4 pt-3 border-t border-slate-200/60 flex items-center justify-between text-xs text-slate-600">
                  <span className="flex items-center gap-1 font-semibold">
                    <Users className="h-3.5 w-3.5 text-slate-400" /> {cohort.memberCount || 0} Students
                  </span>
      ...
      {/* API Key Modal */}
      {isApiKeyModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm" data-testid="api-key-modal">
          <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 sm:p-8 border border-slate-200 space-y-5">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <button onClick={() => { setIsApiKeyModalOpen(false); setIssuedKey(null); }} className="text-slate-400 hover:text-slate-700">✕</button>
            </div>
            ...
            <div className="space-y-2 border-t border-slate-100 pt-4">
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Active Keys</h4>
